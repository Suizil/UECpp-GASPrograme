			...
			if (UUI_SlotBase* InSlotBae = Cast<UUI_SlotBase>(InOperation->Payload))
			{
				...
				case ESlotType::SlotType_EquipmentSlot:
					...
					if (UUI_EquipmentSlot* MyEquipmentSlot = Cast<UUI_EquipmentSlot>(InSlotBae))
					{
						...
						if (Cast<ULSRPGEquipment>(InCharacter->GetLSRPGInventoryComponent()->GetInventoryItemByID(InventoryId))
							|| !InCharacter->GetLSRPGInventoryComponent()->GetInventoryItemInfoByID(InventoryId).IsValid())
						{
