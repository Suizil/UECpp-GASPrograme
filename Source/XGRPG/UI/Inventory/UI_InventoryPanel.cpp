...
void UUI_InventoryPanel::NativeConstruct()
{
	...
	{
		InCharacter->GetXGRPGInventoryComponent()->OnInventoryItemChanged.BindUObject(this, &UUI_InventoryPanel::OnInventoryItemChanged);
		// 去做主动请求服务器更新的操作
...
void UUI_InventoryPanel::NativeDestruct()
{
	...
	if (AXGRPGCharacterBase* InCharacter = UI_GetXGRPGCharacterBase())
	{
		InCharacter->GetXGRPGInventoryComponent()->OnInventoryItemChanged.Unbind();
	}
}
...
void UUI_InventoryPanel::OnInventoryItemChanged(const TArray<FXGRPGInventoryItem>& InInventoryItems)
{
	//第一次更新创建元素
