...
void UUI_CharacterInfo::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	if (AXGRPGCharacterBase* InCharacter = UI_GetXGRPGCharacterBase())
	{
		...
		if (UXGRPGAbilitySystemComponent* InASC = InCharacter->GetXGRPGAbilitySystemComponent())
		{

			if (const UXGRPGCharacterAttributeSet* InAttribute =
			Cast<UXGRPGCharacterAttributeSet>(InASC->GetAttributeSet(UXGRPGCharacterAttributeSet::StaticClass())))
			{
				if (Health)
				{
